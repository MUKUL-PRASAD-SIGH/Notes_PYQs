Program Name : TABLE (TABLE.DLL)
Created: June 15, 1999
Required: Visual Basic 6.0, SP2

NOTE: All length values in this example are in inch units.
...
This example uses the hard coded table values to define the PZL value for bends in Sheet Metal.
If the radius or thickness is not defined in the table then the program computes the value using a 
standard formula.

Even though this example has hardcoded numbers you can easily edit the program 
so that it will use your standard values or formulas. The program defines a public OLE Server called PZL.
